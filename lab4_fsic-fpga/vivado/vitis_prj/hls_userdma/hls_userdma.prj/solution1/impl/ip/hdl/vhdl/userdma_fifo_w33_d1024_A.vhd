...
entity userdma_fifo_w33_d1024_A is
    generic (
...
architecture arch of userdma_fifo_w33_d1024_A is

    -- component
    component userdma_fifo_w33_d1024_A_ram is
    generic (
        MEM_STYLE  : string  := "auto";
        DATA_WIDTH : natural := 33;
...
begin
----------------------- Instantiation -----------------------
    U_userdma_fifo_w33_d1024_A_ram : userdma_fifo_w33_d1024_A_ram
    generic map (
        MEM_STYLE  => MEM_STYLE,
    ...
    process (clk) begin
        ...
        if clk'event and clk = '1' then
            if reset = '1' then
                ...
                if (push = '1') then
                    num_data_valid <= mOutPtr + 1;
                else
                    num_data_valid <= mOutPtr;
                end if;
            elsif not (if_read_ce = '1' and if_read = '1') and dout_vld = '1' then
                if (push = '1') then
                    num_data_valid <= mOutPtr + 2;
                else
                    num_data_valid <= mOutPtr + 1;
                end if;
            else
...
entity userdma_fifo_w33_d1024_A_ram is
    generic (
        MEM_STYLE   : string  := "auto";
        DATA_WIDTH  : integer := 33;
        ADDR_WIDTH  : integer := 10;
...
architecture arch of userdma_fifo_w33_d1024_A_ram is
